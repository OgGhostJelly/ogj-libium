...
type LegacyFilter = filters::Filter;
type NewFilter = super::structs::Filters;

pub fn migrate_filters(
    legacy_filters: Vec<LegacyFilter>,
) -> std::result::Result<NewFilter, MigrateError> {
    let mut filters = NewFilter::empty();
    for filter in legacy_filters {
        filters = filters.concat(filter.try_into()?);
    }
    Ok(filters)
}
